...
def getProjectnr():
    ...
    psxpath = str(workingdir) + "/MetashapeOutput"+ projection + "_" + Zoption + ".psx"
    #psxpath aanpassen afhankelijk van de gekozen projectie
    print("workdir: " +str(workingdir))
...
def m10_exportPLY():
    ...
    chunk.exportModel(path= os.path.join(workingdir, filename),
                      binary=False,
...
def m11_exportOrtho():
    ...
    chunk.exportRaster(path=os.path.join(workingdir, filename), format=Metashape.RasterFormatTiles, image_format=Metashape.ImageFormatNone,
...
def m13_exportPCD():
    ...
    chunk.exportPoints(path=os.path.join(workingdir, filename), source_data=Metashape.DenseCloudData, binary=True, save_normals=True,
