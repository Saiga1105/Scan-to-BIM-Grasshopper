...
namespace Python.Runtime
{
    ...
    public class BooleanType : PyClrTypeBase
    {
        ...
        public override PyObject ToPython(object clrObj)
        {
            //return new PyBoolean(Convert.ToString(clrObj)); // not sure if this works
    ...
    public class Point3dType : PyClrTypeBase
    {
        public Point3dType()
            : base("float", typeof(Rhino.Geometry.Point3d))
        {
        }

        public override object ToClr(PyObject pyObj)
        {
            return new Point3d(pyObj[0].As<double>(), pyObj[1].As<double>(), pyObj[2].As<double>());                
        }

        public override PyObject ToPython(object clrObj)
        {
            var point = (Point3d)clrObj;
            var pyFloat= new PyFloat[3];
            pyFloat[0] = new PyFloat(Convert.ToDouble(point.X));
            pyFloat[1] = new PyFloat(Convert.ToDouble(point.Y));
            pyFloat[2] = new PyFloat(Convert.ToDouble(point.Z));
            return pyFloat.ToPython();
        }
    }
